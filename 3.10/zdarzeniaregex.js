...
function haslosila(){
    ...
    if (regLiczby.test(elPass1.value)) {licznik++;}
    if (licznik==1) {elSila.textContent = 'Słabe'; elSila.style.color = 'red';}
    else if (licznik==2) {elSila.textContent = 'Średnie'; elSila.style.color = 'orange';}
    else if (licznik>=3) {elSila.textContent = 'Dobre'; elSila.style.color = 'green';}
...
elPass1.addEventListener('input',haslosila);
...
//zad dom przy haśle wyświetl czy hasło jest słabe/dobre/silne. wyświetlenie ma być obrazkiem. Kolor zielony silne hasło, pomarańczowy dobre, czerwony słabe hasło (bar)
